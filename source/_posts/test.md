...
* 同时盒模型允许将其它元素和周围元素边框之间的空间放置元素

* 外边框margin 清楚边框外的区域，外边距是透明的，透明的部分没有意义，由主观的个体进行定义
* 边框border 围绕在内边距和内容外的边框
* 内边框padding 清除内容周围的区域，内边距是透明的
* 内容content 盒子的内容，显示文本和图像
* 为了正确设置元素在所有浏览器中的宽度和高度，需要去了解盒模型的工作原理

* 图解说明:

<img src="/images/盒子模型.png" width="40%" height="40%">

* 元素的宽度和高度
> 在指定完一个CSS元素的宽度和高度属性时，此时只是设置了内容区域的宽度和高度
> 如果想要完全大小的元素还必须添加填充，边框和边距

* 比如有这样一列配置
> 总宽度: width: 300px;
> 边框: border: 25px solid green;
> 内边框: padding: 25px;
> 外边距: margin: 25px;
...
* 实例: 在只有250像素的空间的情况下设置总宽度为250像素的元素
> 总宽度 width: 220px;
> 内边距 padding: 10px;
> 边框 border: 5px solid gray;
> 外边距 margin: 0;
> 220px+20px+10px+0=250px

* 同时也可以将所有的HTML元素看作是一个盒子
* 此概念是网页设计以及布局的核心概念
...
* 外边距 margin
> 清除周围的(外边框)元素区域，margin没有背景颜色，是完全透明的
> 可以单独改变元素的上，下，左，右边距，也可以一次性改变所有的属性

* margin属性可以有一到四个值
> 默认从左到右顺序的数字分别表示: 上边距，右边距，下边距，左边距
> 如果只有三个值将是上边距，左右边距，下边距
> 如果只有两个值将是上下边距，左右边距
> 只有一个值时指向所有单位

* 填充/内边距 padding
> 用于定义元素边框与元素内容之间的空间，即上下左右的内边距
> 当元素的填充内边距被清除时，所释放的区域会受到元素背景颜色的填充
> 单独使用padding属性可以改变上下左右的填充

* margin和padding正常的格式是: margin-top/margin-bottom/margin-right/margin-left
> 可以直接缩写为margin与padding，单位是px，结尾需要分号

---

**尺寸(Dimension)**

* 尺寸属性用于控制元素的高度和宽度，同时允许增加行间距
...
<img src="/images/轮廓.png" width="40%" height="40%">

* 外边距&内边距示意图片:

<img src="/images/外边距&内边距.png" width="40%" height="40%">

---

* 个人理解

* 网页中对于元素布局的本质就是选中元素在基于盒模型的前提下，与各种边框中的距离
* 类似于在一张白纸上画一个点，这个点任何向量单位所指向方向的距离于纸张边缘的长度

---

**关于浏览器兼容性的问题:**

* 一旦为页面设置了恰当的DID(文档类型定义)，大多数浏览器都会按照上面的图示来呈现内容
> 然而IE 5和6的呈现却是不正确的
